...
function list(options, config) {
    ...
    // hack to actually use the project configuration
    readJson(defaultConfig.cwd, {
        assume: { name: path.basename(defaultConfig.cwd) || 'root' }
    })
    .spread(function (json, deprecated, assumed) {
        config = mout.object.merge(defaultConfig, json);
